...
export class AuthEffects {
        ...
        this.service
          ...
          .pipe(
            ...
            catchError((errorResponse: HttpErrorResponse) =>
              of(authActions.signInFailure({ errors: errorResponse.error }))
            )
          )
      )
    )
  ...
  resetPassword$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ code, email, password, confirmPassword }) =>
        this.service
          .resetPassword({
            code,
            email,
  ...
  avatarUpload$ = createEffect(() =>
    this.actions$.pipe(
      ofType(authActions.avatarUpload),
      switchMap(({ file }) =>
        this.profileService.uploadAvatar(file).pipe(
          map(({ accessToken }) => authActions.avatarUploadSuccess({ accessToken })),
          catchError((errorResponse: HttpErrorResponse) => of(avatarUploadFailure({ errors: errorResponse.error })))
        )
      )
    )
  );
  ...
  /* TODO: Should be moved to own store (profile) */
  avatarRemove$ = createEffect(() =>
    this.actions$.pipe(
      ofType(authActions.avatarRemove),
      switchMap(() =>
        this.profileService.removeAvatar().pipe(
          map(({ accessToken }) => authActions.avatarRemoveSuccess({ accessToken })),
          catchError((errorResponse: HttpErrorResponse) => of(avatarRemoveFailure({ errors: errorResponse.error })))
        )
      )
    )
  );
  ...
  avatarUpdateSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(authActions.avatarUploadSuccess),
        tap(({ accessToken }) => {
          this.jwtTokenService.setCurrentUser(accessToken);
          const currentUser: JwtPayloadInterface = this.jwtTokenService.getUser();
          this.store.dispatch(authActions.getCurrentUserSuccess({ currentUser }));
  ...
  /* TODO: Should be moved to own store (profile) */
  avatarRemoveSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(authActions.avatarRemoveSuccess),
        tap(({ accessToken }) => {
          this.jwtTokenService.setCurrentUser(accessToken);
          const currentUser: JwtPayloadInterface = this.jwtTokenService.getUser();
          this.store.dispatch(authActions.getCurrentUserSuccess({ currentUser }));
          this.notificationsService.success('Success!', `Avatar removed!`);
        })
      ),
    { dispatch: false }
  );
